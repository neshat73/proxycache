...
### Инструкция по запуску (простым языком)

#### Шаг 1: Запуск нейросетевого движка (llama.cpp)

Сначала нужно запустить сам сервер llama.cpp, указав ему, сколько «комнат»-слотов создать и где хранить файлы кэшей.

```bash
./server -m ваша-модель.gguf -np 4 --slot-save-path /путь/к/папке/кэшей
```
...
### Описание параметров (простым языком)
